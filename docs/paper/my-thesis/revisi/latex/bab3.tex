...
Lapisan antarmuka dibangun menggunakan Next.js 14 dengan App Router dan TypeScript.
Komponen utama:
\begin{itemize}
  ...
  \item \textbf{Clerk Middleware:} Setiap halaman dilindungi oleh \textit{middleware}
    yang memvalidasi sesi Clerk. Token JWT (RS256) disisipkan ke header
    \texttt{Authorization: Bearer <token>} pada setiap permintaan API ke backend.
    Algoritma RS256 (asymmetric) dipilih agar backend dapat memverifikasi token
    menggunakan \textit{public key} tanpa memerlukan \textit{shared secret}.
